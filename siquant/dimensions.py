...
angle_t = solid_angle_t = strain_t = ratio_t = SIDimensions()
...
impulse_t = momentum_t = dim_mul(force_t, time_t)
pressure_t = stress_t = dim_div(force_t, area_t)
hydrostatic_pressure_t = dim_mul(density_t, dim_mul(acceleration_t, distance_t))
surface_tension_t = stiffness_t = dim_div(force_t, distance_t)

energy_t = work_t = heat_t = dim_mul(force_t, distance_t)
power_t = dim_div(energy_t, time_t)

charge_t = dim_mul(current_t, time_t)
potential_t = dim_div(energy_t, current_t)
capacitance_ = dim_div(charge_t, potential_t)
resistance_ = impedance_t = reactance_t = dim_div(potential_t, current_t)
...
entropy_t = heat_capacity_t = dim_div(energy_t, temperature_t)
specific_entropy_t = specific_heat_capacity_t = dim_div(entropy_t, mass_t)
